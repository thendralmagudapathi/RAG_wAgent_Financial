#!/usr/bin/env python3
# Clean retriever implementation (use this if retriever.py is corrupted)

import os
import glob
from parser import extract_text_from_html, guess_company_name, chunk_text_with_lines
...
class RAGBuilder:
    ...
    def build_from_dir(self, data_dir: str):
        ...
        for f in files:
            ...
            chunks = chunk_text_with_lines(text, max_chars=1000)
            if not chunks:
                continue
            ...
            metadatas = [
                {
                    ...
                    'source': f,
                    'chunk_id': i,
                    'start_line': c.get('start_line', None),
                    'page': c.get('page', None) if isinstance(c, dict) else None,
                }
