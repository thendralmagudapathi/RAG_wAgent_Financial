...
## Next steps / tips
- If you need improved speed with `tinyllama` on CPU, consider:
  - Using a smaller top-K (1 or even 0 for routing-only tests).
  - Running on a machine with more CPU cores or using a model compiled/quantized for faster inference.
  - Reducing the verbosity / response length in the UI.

If you'd like, I can:
- Add a minimal `requirements.txt` based on imported modules in the repo.
- Add a small troubleshooting script that checks Ollama status and a one-shot LLM health call.

---

Happy debugging — tell me which next improvement you want and I'll implement or test it for you.
